...
public class SqlXmlKit {
			...
			if (clounm.equals("#" + clounm2 + "#")) { // 数值型，可以对应处理int、long、bigdecimal、double等等
				...
				if (data != null) {
					if (data instanceof Object[]) {// 数组
						...
						sql = sql.replace(clounm, sb.substring(1));
					} else if (data instanceof Iterator) {// list iterator等
				...
				if (data != null) {
					if (data instanceof Object[]) {// 数组
						...
						for (int i = 0; i < arrData.length; i++) {
							sb.append(",?");
							Object dt = arrData[i];
							dt = dt instanceof Date ? DateTools
									.formatDateTime((Date) dt) : dt;
						}
						sql = sql.replace(clounm, sb.substring(1));
					} else if (data instanceof Map) {// Map
						Map<String, ?> mapData = (Map<String, ?>) data;
						...
						while (iterator.hasNext()) {
							...
							sb.append(",?");
							dataItm = dataItm instanceof Date ? DateTools
									.formatDateTime((Date) dataItm) : dataItm;
						}
						sql = sql.replace(clounm, sb.substring(1));
					} else {// 普通对象
						data = data instanceof Date ? DateTools
								.formatDateTime((Date) data) : data;
						sql = sql.replace(clounm, "?");
					}
				} else {
					sql = sql.replace(clounm, "NULL");
				...
				if (data != null) {
					if (data instanceof Object[]) {// 数组
						...
						for (int i = 0; i < arrData.length; i++) {
							sb.append(",?");
							Object dt = arrData[i];
							list.add(dt instanceof Date ? DateTools
									.formatDateTime((Date) dt) : dt);
						}
						sql = sql.replace(clounm, sb.substring(1));
					} else if (data instanceof Map) {// Map
						...
						while (iterator.hasNext()) {
							Object dataItm = iterator.next();
							sb.append(",?");
							list.add(dataItm instanceof Date ? DateTools
									.formatDateTime((Date) dataItm) : dataItm);
						}
						sql = sql.replace(clounm, sb.substring(1));
					} else {// 普通对象
						sql = sql.replace(clounm, "?");
						list.add(data instanceof Date ? DateTools
								.formatDateTime((Date) data) : data);
					}
				} else {
					sql = sql.replace(clounm, "NULL");
				...
				if (data != null) {
					if (data instanceof Object[]) {// 数组
						...
						while (iterator.hasNext()) {
							...
							dataItm = dataItm instanceof Date ? DateTools
									.formatDateTime((Date) dataItm) : dataItm;
							list.add(clounm4.replace(clounm2,
									dataItm.toString()));
						}
						sql = sql.replace(clounm, sb.substring(1));
					} else {// 普通对象
						data = data instanceof Date ? DateTools
								.formatDateTime((Date) data) : data;
						String clounm4 = clounm.replace("#", "")
								.replace("'", "")
								.replace(clounm2, data.toString());
						list.add(clounm4);
						sql = sql.replace(clounm, "?");
					}
				} else {
	...
	public static void init() {
		File file = new File(SqlXmlKit.class.getClassLoader().getResource("")
				.getFile());
				...
				for (Iterator iterTemp = root.elementIterator(); iterTemp
					...
					if (element.getName().toLowerCase().equals("sql")) {
						...
						if (sqlMap.containsKey(key)) {
							log.error("sql xml文件" + fileName + "的sql语句" + key
									+ "的存在重复命名空间和ID");
							continue;
						}
						...
						sqlMap.put(key, sql);
						log.debug("sql文件名：" + fileName + " sql key: " + key
								+ " sql内容：" + sql);
					}
				}
			}
